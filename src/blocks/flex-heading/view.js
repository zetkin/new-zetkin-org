const cn = ( suffix = '' ) => `wp-block-zetkin-flex-heading${ suffix }`;

function adjustFontSize() {
	// Select all elements with the class '.zetkin_flexHeader'
	const containers = document.querySelectorAll( cn( ' p' ) );

	// Iterate over each element
	containers.forEach( function ( container ) {
		const fontSize = container.offsetWidth * 0.075; // Calculate font size as 5% of container's width
